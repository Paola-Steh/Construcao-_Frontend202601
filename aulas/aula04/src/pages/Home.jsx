// useEffect é como um evento disparado pela função, sendo a última ação executada
// usado quando se usa muitos dados
// se ve que ele foi usado no "carregarDados" 

import { useState, useEffect } from "react";

function Home() {
  const [carregando, setCarregando] = useState(true); // para usar o useEffect
  const [ tarefas, setTarefas ] = useState([]);

  const carregarDados = async() => { 
    await new Promise((resolve) => setTimeout(resolve, 3000)); // para usar o useEffect
    setTarefas ([
    {id: 1, nome: "Fazer o trabalho", status: "aberta"},
    {id: 2, nome: "Estudar API REST", status: "aberta"},
    {id: 3, nome: "Entregar as práticas", status: "pendente"},
    {id: 4, nome: "Revisar conteúdo", status: "pendente"},
    {id: 5, nome: "Assistir vídeo-aulas", status: "concluida"},
    ]);
  };

  useEffect(() => {  //  useEffect o principal
    const disparar = async () => {
      await carregarDados();
        setCarregando(false);
    };
     disparar();
  }, []);

  return (
    <>
      <h1>Página Inicial</h1>
      <h2>Minhas Tarefas do Dia</h2>
      {carregando ? ( //  estado para o useEffect
                                                      // py > altura | px > largura
      <> 
        <p>Aguarde...</p>
        <table className="border border-gray-300 rounded">
          <thead>
            <tr className="bg-gray-300">
              <th className="py-4 px-8"></th> 
              <th className="py-4 px-8"></th>
              <th className="py-4 px-8"></th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td><div className="my-1 mx-1 py-0.5 rounded bg-gray-300"></div></td>
              <td><div className="my-1 mx-1 py-0.5 rounded bg-gray-300"></div></td>
              <td><div className="my-1 mx-1 py-0.5 rounded bg-gray-300"></div></td>
            </tr>
            <tr>
              <td><div className="my-1 mx-1 py-0.5 rounded bg-gray-300"></div></td>
              <td><div className="my-1 mx-1 py-0.5 rounded bg-gray-300"></div></td>
              <td><div className="my-1 mx-1 py-0.5 rounded bg-gray-300"></div></td>
            </tr>
            <tr>
              <td><div className="my-1 mx-1 py-0.5 rounded bg-gray-300"></div></td>
              <td><div className="my-1 mx-1 py-0.5 rounded bg-gray-300"></div></td>
              <td><div className="my-1 mx-1 py-0.5 rounded bg-gray-300"></div></td>
            </tr>
          </tbody>
        </table>
      </>
      ) : (
      <table>
        <thead>
          <tr>
            <th>ID</th>
            <th>Tarefa</th>
            <th>Status</th>
          </tr>
        </thead>
        <tbody>
        {tarefas.map((item, index) => (
          <tr key={index}>
            <td>{item.id}</td>
            <td>{item.nome}</td>
            <td>{item.status}</td>
          </tr>
        ))}
        </tbody>
      </table>
      )}
    </>
  );
}

export default Home;
